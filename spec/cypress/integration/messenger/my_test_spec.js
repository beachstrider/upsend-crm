  ...
  cy.appEval('App.last').then((results) => {
    ...
    .then((results) => {
        cy.visit(`/tester/${record.key}${visitor ? "?sessionless=true" : ''}`).then(()=>{
          ...
          if(nullFrame)
            return cy.get('iframe:first').should('not.exist');
 
          cy.get('iframe:first')
          .then(function ($iframe) {
              const $body = $iframe.contents().find('body')
              cb && cb($body, cy)
              //expect($body.find("#upsend-prime").length).to.equal(1)
          })
...
describe('Upsend Messenger', function() {
  beforeEach(() => {
  ...
  it('renders messenger on anonimous user creating a app user', function() {
    createApp().then((results) => {
      ...
      cy.visit(`/tester/${record.key}`).then(()=>{
        //cy.wait(10000)
        //cy.get('#UpsendMessengerRoot').should('be.visible')      
        cy.get('iframe:first')
        .then(function ($iframe) {
            const $body = $iframe.contents().find('body')
    ...
    cy.contains("Upsend tester site")
  })
