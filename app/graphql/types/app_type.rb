...
module Types
  class AppType < Types::BaseObject
    ...
    field :agent_ses_identities, [Types::SesIdentityType], null: true
    field :plan, Types::PlanType, null: true
    field :available_plan_categories, [Types::PlanType], null: true
    field :billing, Types::JsonType, null: true
    field :can_add_agents, Types::JsonType, null: true
    field :can_add_contacts, Types::JsonType, null: true
    field :email_forwarding_address, String, null: true

    def email_forwarding_address
      object.email_forwarding_address
    end

    def can_add_agents
      total_agents = object.agents.size
      max_limit = object.active_plan.seats
      (total_agents < max_limit) ? true : false 
    end

    def can_add_contacts
      #We will not restrice them to add contacts. 
      #Add on charges will be applied in stripe metered billing
      #For cases like free plan limit exhausted then we will block from here
      true
    end

    def billing
      obj = object.billing.blank? ? object.build_billing : object.billing
      h = obj.attributes
      h[:last4] = object.active_subscription.last4 rescue ''
      h[:brand] = object.active_subscription.card_type.capitalize rescue ''
      h
    end


    def available_plan_categories 
      Plan.where(plan_type: [nil,'base']).order("amount_cents asc")
    end
    ...
    def agent_ses_identities
      object.ses_identities
    end
