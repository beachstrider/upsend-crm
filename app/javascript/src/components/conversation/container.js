...
class ConversationContainerShow extends Component {
  ...
  componentDidUpdate(PrevProps, PrevState) {
    ...
    if (
      PrevProps.conversation.collection &&
      this.props.conversation.collection &&
      this.props.conversation.collection.length !=
        PrevProps.conversation.collection.length
    ) {
  ...
  handleScroll = (e) => {
    ...
    if (element.scrollTop === 0) {
      // on top
      if (
        this.props.conversation.meta.next_page &&
  ...
  scrollToItem = (item) => {
    if (item) {
      this.refs.overflow.scrollTop = document.querySelector(
        `#message-id-${item}`
      ).offsetHeight;
    } else {
      this.scrollToLastItem();
  ...
  render() {
    return (
      <GridElement grow={2} style={{overflow: 'hidden'}}>
                ...
                <IconButton
                  onClick={() =>
                    this.props.history.push(
                      `/apps/${this.props.app.key}/conversations`
                    )
                  }
                >
                  <BackIcon />
              ...
              <ConversationButtons>
                ...
                {this.props.conversation.state != "closed" ? (
                  <Tooltip title="Close conversation">
                    <IconButton
                      onClick={() => {
                        this.updateConversationState("close");
                      }}
                    >
                      <CheckIcon />
                    </IconButton>
                ...
                {this.props.conversation.state != "opened" ? (
                  <Tooltip title="Reopen conversation">
                    <IconButton
                      onClick={() => {
                        this.updateConversationState("reopen");
                      }}
                    >
                      <InboxIcon />
                    </IconButton>
                ...
                <Tooltip
                  title={
                    !this.props.conversation.priority
                ...
                >
                  <IconButton onClick={this.toggleConversationPriority}>
                    <PriorityHighIcon
                      color={
                        this.props.conversation.priority ? "primary" : "inherit"
                      }
                    />
                  </IconButton>
                </Tooltip>
            ...
            >
              ...
              >
                {this.props.conversation.collection.map((o, i) => {
                  ...
                  const userOrAdmin =
                    !isReplied && o.appUser && o.appUser.kind === "agent"
                      ? "admin"
                      : "user";
                    ...
                    >
                      ...
                      >
                        {o.appUser && (
                          <ChatAvatar
                            onClick={(e) =>
                              this.props.showUserDrawer(appuserId)
                            }
                            className={userOrAdmin}
                          >
                            <img
                        ...
                        <StatusItem>
                          {userOrAdmin != "admin" ? (
                            <Moment className="chat-status-user" fromNow>{o.createdAt}</Moment>
                          ) : (
