...
class ApiController < ActionController::API
  ...
  def merge_user_data(model)
    ...
    model.update(
          properties: @user_data[:properties].except(:ws,:app_id,:domain,:lang),
          lang: I18n.locale
        ) if @user_data[:properties].present?
  end
  ...
  def get_user_from_unencrypted
    user_data = request.headers['HTTP_USER_DATA']
    if user_data.present?
      json_response = JSON.parse(user_data).deep_symbolize_keys
      resp = {email: json_response[:email], properties: json_response}
    end 
      resp.present? ? resp : {}
  rescue StandardError
    nil
