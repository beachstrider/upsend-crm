...
module MessageApis
  class Slack
    ...
    def post_message(message, blocks, options={})
      ...
      data = {
        "channel": options[:channel] || @keys['channel'] || 'upsend_channel',
        "text": message,
        "blocks": blocks,
    ...
    def create_channel(name='upsend_channel', user_ids="")
      authorize!
    ...
    def notify_added(conversation)
      ...
      links = "*<#{user_url}|#{conversation.main_participant.display_name}>* <#{conversation_url}|view in upsend>"

      data = {
        "channel": @keys['channel'] || 'upsend_channel',
        "text": 'New conversation from Upsend',
        "blocks": [
          {
            "type": "section",
            "text": {
    ...
    # triggered when a new upsend message is created
...
+ usuario crea conversacion
  + se manda mensaje por canal upsend
...
cuando genera mensaje desde upsend
